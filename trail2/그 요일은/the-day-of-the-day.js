...
// 1월 1일부터 각각 며칠 차이가 나는지 구하기
function totalDaysFromJan1(m, d) {
    let totalDays = 0
    for (let i = 0; i < m; i++) {
...
let days1 = totalDaysFromJan1(m1, d1);
let days2 = totalDaysFromJan1(m2, d2);

let leftDays = days2 - days1 + 1;

let numTotal = parseInt(leftDays / 7)

let weekInstans = ((leftDays % 7) + 7) % 7

for (let i = 0; i < weekInstans; i++) {
    if (week[i] === A) {
        numTotal = numTotal + 1
    }
}
